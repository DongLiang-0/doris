...
public class AvroJNIScanner extends JniScanner {
    ...
    protected int getNext() throws IOException {
        ...
        for (; numRows < getBatchSize(); numRows++) {
            ...
            for (int i = 0; i < requiredFields.length; i++) {
                // Object fieldData = rowInspector.getStructFieldData(rowRecord, structFields[i]);
    ...
    protected TableSchema parseTableSchema() throws UnsupportedOperationException {
        ...
        for (int i = 0; i < schemaFields.size(); i++) {
            Schema avroSchema = schemaFields.get(i).schema();
            String columnName = schemaFields.get(i).name().toLowerCase(Locale.ROOT);
